...
class SecureMonarchSession:
    ...
    def load_token(self) -> Optional[str]:
        """Load the authentication token. Priority: env var > keyring."""
        # 1. Check MONARCH_TOKEN environment variable (for cloud deployment)
        env_token = os.getenv("MONARCH_TOKEN")
        if env_token and env_token.strip():
            logger.info("✅ Token loaded from MONARCH_TOKEN environment variable")
            return env_token.strip()

        # 2. Fall back to system keyring (for local Windows)
        try:
